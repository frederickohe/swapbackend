...
class Listing(Base):
    ...
    # Optional add-on services (fees applied at swap transaction time)
    wish_finding: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    budget_negotiation: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    budget_amount: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    collection_assistance: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
